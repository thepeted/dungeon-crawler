...
export default () => {
	...
	const createRandomRoom = (
		coords = c.STARTING_ROOM_POSITION,
		sizeRange = c.ROOM_SIZE_RANGE) => {
		const [a, b] = sizeRange;
		const [x, y] = coords;
		return {
			height: _.random(a, b),
	...
	const seedRooms = []; // seed rooms will be populated by placeRoom function
	...
	const placeRoom = (room) => {
		...
		const tempWorld = _.cloneDeep(world);
	...
	const cleanedWorld = world.map(row => {
		return row.map(cell => {
			if (cell === 0) {
				return {
					type: 0,
